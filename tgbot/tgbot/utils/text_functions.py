...
async def open_profile_user(bot: Bot, user_id: Union[int, str]):
    ...
    bonus_text = ""
    if get_user.user_bonus_balance > 0:
        bonus_text = f"\n🎁 Бонусный баланс: <code>{get_user.user_bonus_balance}$</code> <i>(только для покупки товаров)</i>"

    send_text = ded(f"""
        <b>👤 Ваш профиль:</b>
        ➖➖➖➖➖➖➖➖➖➖
        🆔 ID: <code>{get_user.user_id}</code>
        💰 Баланс: <code>{get_user.user_balance}$</code>{bonus_text}
...
async def open_profile_admin(bot: Bot, user_id: int, get_user: UserModel):
    ...
    send_text = ded(f"""
        ...
        ▪️ Баланс: <code>{get_user.user_balance}$</code>{bonus_text}
