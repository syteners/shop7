...
async def admin_export_users(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    await call.message.edit_text("<b>📥 Генерирую файл с пользователями...</b>")
    
    get_users = Userx.get_all()
    
    if not get_users:
        return await call.message.edit_text("<b>❌ Пользователи не найдены</b>")
    ...
    # Заголовки
    writer.writerow([
        'ID', 'Username', 'Имя', 'Баланс', 
        'Всего пополнено', 'Всего выдано', 
        'Дата регистрации'
    ])
    ...
    for user in get_users:
        writer.writerow([
            user.user_id,
            f"@{user.user_login}" if user.user_login else "Нет",
            user.user_name,
            f"${user.user_balance}",
            f"${user.user_refill}",
            f"${user.user_give}",
            convert_date(user.user_unix, full=False)
        ])
    ...
    csv_file = BufferedInputFile(
        csv_content.encode('utf-8-sig'),  # utf-8-sig для корректного отображения в Excel
        filename=f"users_export_{get_unix()}.csv"
    )
    ...
    await call.message.answer_document(
        ...
        caption=f"<b>📥 Экспорт пользователей</b>\n"
                f"➖➖➖➖➖➖➖➖➖➖\n"
                f"▪️ Всего пользователей: <code>{len(get_users)}</code>\n"
                f"▪️ Дата экспорта: <code>{convert_date(get_unix())}</code>"
    )
