...
from tgbot.keyboards.inline_admin_cryptobot import payment_cryptobot_finl
...
async def payment_methods_edit(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    if way_pay == "QIWI":
        ...
        if way_status == "True" and get_payment.cryptobot_token == "None":
            return await call.answer("❗ Добавьте CryptoBot кошелёк перед включением Способов пополнений", True)

        Paymentsx.update(way_cryptobot=way_status)
...
async def payment_yoomoney_edit_token(message: Message, bot: Bot, state: FSM, arSession: ARS):
    ...
    status, response, token = YoomoneyAPI(
        bot=bot,
        arSession=arSession,
...
async def payment_cryptobot_edit(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    await call.message.edit_text(
        ded(f"""
            <b>💎 Для изменения CryptoBot кошелька</b>
            ▪️ Получите API токен в @CryptoBot (команда /api)
            ▪️ Отправьте полученный токен боту
            🔗 Инструкция: https://help.crypt.bot/crypto-pay-api
        """),
        disable_web_page_preview=True,
    )
...
@router.message(F.text, StateFilter("here_cryptobot_token"))
async def payment_cryptobot_edit_token(message: Message, bot: Bot, state: FSM, arSession: ARS):
    cryptobot_token = message.text.strip()
    ...
    if status:
        Paymentsx.update(cryptobot_token=cryptobot_token)
