...
from tgbot.routers.admin import admin_menu, admin_functions, admin_payment, admin_products, admin_settings, admin_promocode
from tgbot.routers.user import user_menu, user_transactions, user_products, user_promocode
...
def register_all_routers(dp: Dispatcher):
    ...
    admin_promocode.router.message.filter(IsAdmin())  # Работа роутера только для админов
    ...
    dp.include_router(user_promocode.router)  # Юзер роутер промокодов
    ...
    dp.include_router(admin_promocode.router)  # Админ роутер промокодов
