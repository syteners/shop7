...
async def user_buy_open(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    # Проверка, имеется ли на балансе пользователя достаточно средств (основной + бонусный)
    total_balance = get_user.user_balance + get_user.user_bonus_balance
    if total_balance < int(get_position.position_price):
    ...
    if get_position.position_price != 0:
        get_count = round(int(total_balance / get_position.position_price), 2)
    ...
    if get_items == 1:
        ...
        await call.message.answer(
            ded(f"""
                ...
                ▪️ Бонусный баланс: <code>{get_user.user_bonus_balance}$</code>
            """),
...
async def user_buy_count(message: Message, bot: Bot, state: FSM, arSession: ARS):
    ...
    # Максимальное количество товаров к покупке, подстроенные под баланс пользователя (основной + бонусный)
    total_balance = get_user.user_balance + get_user.user_bonus_balance
    if get_position.position_price != 0:
        get_count = int(total_balance / get_position.position_price)
    ...
    # Если баланс пользователя (основной + бонусный) меньше, чем цена покупки
    if total_balance < amount_pay:
        return await message.answer(
...
async def user_buy_confirm(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    ...
    # Проверка баланса пользователя (основной + бонусный) и общей суммы покупки
    total_balance = get_user.user_balance + get_user.user_bonus_balance
    if total_balance < purchase_price:
    ...
    # Списываем сначала с бонусного баланса, потом с основного
    bonus_used = 0
    balance_used = 0
    
    if get_user.user_bonus_balance >= purchase_price:
        # Хватает бонусного баланса
        bonus_used = purchase_price
        Userx.update(
            get_user.user_id,
            user_bonus_balance=round(get_user.user_bonus_balance - purchase_price, 2),
        )
    elif get_user.user_bonus_balance > 0:
        # Используем весь бонусный баланс и часть основного
        bonus_used = get_user.user_bonus_balance
        balance_used = purchase_price - get_user.user_bonus_balance
        Userx.update(
            get_user.user_id,
            user_bonus_balance=0,
            user_balance=round(get_user.user_balance - balance_used, 2),
        )
    else:
        # Используем только основной баланс
        balance_used = purchase_price
        Userx.update(
            get_user.user_id,
    ...
    # Формируем сообщение о списании
    payment_info = ""
    if bonus_used > 0 and balance_used > 0:
        payment_info = f"\n▪️ Списано с бонусного: <code>{bonus_used}$</code>\n▪️ Списано с основного: <code>{balance_used}$</code>"
    elif bonus_used > 0:
        payment_info = f"\n▪️ Списано с бонусного баланса: <code>{bonus_used}$</code>"
    else:
        payment_info = f"\n▪️ Списано с основного баланса: <code>{balance_used}$</code>"
    ...
    await call.message.answer(
        ded(f"""
            ...
            ▪️ Товар: <code>{get_position.position_name} | {purchase_count}шт | {purchase_price}$</code>{payment_info}
